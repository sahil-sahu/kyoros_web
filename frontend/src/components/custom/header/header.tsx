...
export default function NavBox({title}:{title:string|null|undefined}) {
    ...
    return <nav className="flex items-center justify-between flex-wrap p-6 border-b border-gray-300">
        <Link href={'/'} className="flex items-center overflow-hidden flex-shrink-0 text-white md:mr-6">
            <Image className="object-cover" src={Logo} height={50} alt="Kyoros"/>
        </Link>
        <div className="flex items-center text-lg font-semibold  flex-shrink-0 md:mr-6">
            <h3>
                {title ?? "Overview"}
            </h3>
        </div>
