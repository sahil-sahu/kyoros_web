...
export default function Login(){
  ...
  React.useEffect(()=>{
    ...
    //   router.push("/"+data);
    // }
    setTimeout(()=>{router.push("/auth/forgot/verify")}, 5000);
  }, [router]);
    ...
    <Card className="w-[400px] bg-white p-8 border-none md:border-solid rounded">
        ...
        <CardContent>
            <div className="flex-row flex gap-2 p-2">
                ...
                <RadioGroup className="flex-row flex flex-1" defaultValue="option-one">
            ...
            <div className="grid w-full items-center gap-4">
                <div className="flex flex-col space-y-1.5">
                    <Input id="email" onChange={(e) => setEmail(e.target.value)} value={email} type="text" placeholder="E-mail (as on account)" />
                </div>
            </div>
        </CardContent>
        <CardFooter className="flex justify-between">
            <Button className="w-[100%] text-center bg-bluecustom" type="submit">Reset</Button>
        ...
        <CardFooter className="flex text-sm text-center justify-between">
