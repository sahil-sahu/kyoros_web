...
const SubscribeNotification = ({fcmSet, Reset}:{fcmSet:boolean; Reset: Dispatch<SetStateAction<boolean>>}) => {
    const { mutate, isPending:isLoading, error, data, isSuccess } = useMutation({mutationFn:setFcm});
    const [loading, setLoading] = useState(false);
    if(isSuccess) Reset(true);
    const requestNotificationPermission = async (e:any) => {
        // if(!isSupported()){
    ...
    return (
        <Dialog open={!fcmSet}>
            ...
            <DialogContent className="sm:max-w-[425px]">
                ...
                <Button id="subscribe" disabled={loading} className={loading?"bg-black":""} variant={"destructive"} >
                    {loading? "Loading...":"Subscribe"}
                    {loading && <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="#fff" className="animate-spin w-6 h-6">
