...
const PrintDialog = ({patientId}:{patientId:string}) =>{
    ...
    const handlePrint = () =>{
        ...
        window.open(`/tracking/print?${currentParams.toString()}`, "_blank");
    }
      ...
      <LocalizationProvider dateAdapter={AdapterDayjs}>
        <Dialog>
          ...
          <DialogContent className="max-h-screen overflow-auto">
            ...
            <DialogFooter>
              {
                download ? (<a className='w-full' download={"logs.pdf"} href={`/tracking/print/download/?url=${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/tracking/print?start=${startDt.toISOString()}&end=${endDt.toISOString()}&params=${params.join(",")}&patientId=${patientId}&freq=${freq}`}>
